...
public class Cube extends GameObject
{
	...
	{
		if (!(gridPosition.x + dx >= -1 && gridPosition.x + dx <= 1))
		{
			return false;
		}
		if (!(gridPosition.y + dy >= -1 && gridPosition.y + dy <= 1))
		{
			return false;
		}
		if (!(gridPosition.z + dz >= -1 && gridPosition.z + dz <= 1))
		{
			return false;
		}
		return true;
	...
	private void clamp(Vector3i gridPosition, int min, int max)
	{
		gridPosition.x = Math.max(min, Math.min(max, gridPosition.x));
		gridPosition.y = Math.max(min, Math.min(max, gridPosition.y));
